...
// ignore_for_file: implementation_imports, always_put_control_body_on_new_line
...
Token? getFieldName(FieldDeclaration decl, String name) {
  for (final VariableDeclaration v in decl.fields.variables) {
...
SyntacticEntity getNodeToAnnotate(Declaration node) {
  final SyntacticEntity? mostSpecific = _getNodeToAnnotate(node);
...
Element? getWriteOrReadElement(SimpleIdentifier node) {
  final Element? writeElement = _getWriteElement(node);
...
bool hasConstantError(LinterContext context, Expression node) {
  final LinterConstantEvaluationResult result = context.evaluateConstant(node);
...
bool isInLibDir(CompilationUnit node, WorkspacePackage? package) {
  ...
  final String? cuPath = node.declaredElement?.library.source.fullName;
  if (cuPath == null) return false;
  final String libDir = path.join(package.root, 'lib');
...
/// `lib` and `bin` dirs.
//
//  TO DO : move into WorkspacePackage
bool isInPublicDir(CompilationUnit node, WorkspacePackage? package) {
  if (package == null) return false;
  final String? cuPath = node.declaredElement?.library.source.fullName;
  if (cuPath == null) return false;
  final String libDir = path.join(package.root, 'lib');
  final String binDir = path.join(package.root, 'bin');
...
bool isPrivate(Token? name) => Identifier.isPrivateName(name?.lexeme ?? '');

/// Returns `true` if the given [ClassMember] is a public method.
bool isPublicMethod(ClassMember m) {
  final Element? declaredElement = m.declaredElement2;
...
bool isSimpleGetter(MethodDeclaration declaration) {
  ...
  final FunctionBody body = declaration.body;
...
File? locatePubspecFile(CompilationUnit compilationUnit) {
  final String? fullName = compilationUnit.declaredElement?.source.fullName;
  if (fullName == null) {
    return null;
  }
  ...
  final File file = resourceProvider.getFile(fullName);

  // Look for a pubspec.yaml file.
  for (final Folder folder in file.parent.withAncestors) {
    final File pubspecFile = folder.getChildAssumingFile('pubspec.yaml');
...
bool _checkForSimpleGetter(MethodDeclaration getter, Expression? expression) {
  if (expression is SimpleIdentifier) {
    final Element? staticElement = expression.staticElement;
    if (staticElement is PropertyAccessorElement) {
      final Element? enclosingElement =
          getter.declaredElement2?.enclosingElement3;
...
bool _checkForSimpleSetter(MethodDeclaration setter, Expression expression) {
  ...
  final Expression leftHandSide = expression.leftHandSide;
  final Expression rightHandSide = expression.rightHandSide;
  if (leftHandSide is SimpleIdentifier && rightHandSide is SimpleIdentifier) {
    final Element? leftElement = expression.writeElement;
    ...
    final Element? rightElement = rightHandSide.staticElement;
    if (rightElement is! ParameterElement) {
      return false;
    }

    final NodeList<FormalParameter>? parameters = setter.parameters?.parameters;
...
List<String> _collectReservedWords() {
  ...
  for (final MapEntry<String, Keyword> entry in Keyword.keywords.entries) {
...
class _ElementVisitorAdapter extends GeneralizingElementVisitor<void> {
  ...
  void visitElement(Element element) {
    final bool visitChildren = processor(element);
    if (visitChildren) {
