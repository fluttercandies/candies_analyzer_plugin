...
final RegExp _identifier =
    RegExp(r'^[_$a-z]+(\d[_a-z\d]*)?$', caseSensitive: false);
...
final RegExp _lowerCaseUnderScore = RegExp(r'^[a-z](?:_?[a-z\d])*$');

@Deprecated('Prefer: ascii_utils.isValidFileName')
final RegExp _lowerCaseUnderScoreWithDots =
...
// * with no two adjacent underscores,
// * and not ending in an underscore.
final RegExp _lowerCaseUnderScoreWithLeadingUnderscores =
...
final RegExp _validLibraryPrefix = RegExp(r'^\$?_*[a-z][_a-z\d]*$');
...
class CamelCaseString {
  ...
  static final RegExp _camelCaseMatcher = RegExp(r'[A-Z][a-z]*');
  ...
  // `_`.
  static final RegExp _camelCaseTester =
      RegExp(r'^_*(?:\$+_+)*[$?A-Z][$?a-zA-Z\d]*$');
  ...
  static String _humanize(String camelCase) => _camelCaseMatcher
      .allMatches(camelCase)
