...
export 'package:analyzer/src/lint/util.dart';
...
const int loggedAnalyzerErrorExitCode = 63;
...
Future<Iterable<AnalysisErrorInfo>> lintFiles(
  ...
  final ErrorWatchingSink errorWatcher = ErrorWatchingSink(errorSink);
  errorSink = errorWatcher;

  final Iterable<AnalysisErrorInfo> errors =
      await linter.lintFiles(filesToLint);
...
int _maxSeverity(List<AnalysisErrorInfo> errors, LintFilter? filter) {
  int max = 0;
  for (final AnalysisErrorInfo info in errors) {
...
class ErrorWatchingSink implements IOSink {
  ...
  @override
  Future<dynamic> get done => delegate.done;
  ...
  @override
  Future<dynamic> flush() => delegate.flush();
